...
# def contrastive_loss(current_logits, current_labels, memory_bank, memory_label, device, multi_label=False):
#     con_loss = 0
#     class_num = current_labels.shape[1]
#     memory_bank = torch.from_numpy(memory_bank)
#     memory_bank = memory_bank.to(device)
...
def train_llm(config):
    ...
    id_to_label = {v: k for k, v in labels_to_ids.items()}
    ...
    #
    #         con_loss = contrastive_loss(outputs_con, batch["labels"].detach().cpu().numpy(), sub_memory_bank, sub_memory_label, device)
    #         # con_loss = contrastive_loss(outputs_con, np.argmax(batch["labels"].detach().cpu().numpy(),axis=1), memory_bank, memory_labels, device)
    #         loss = loss - 0.02 * con_loss
    #
    ...
    best_thresholds = {'background': 0.6000000000000001, 'objective': 0.1, 'method': 0.5, 'result': 0.4, 'conclusion': 0.5, 'other': 0.30000000000000004}
    ...
    pred_original_labels = [id_to_label[pred.index(max(pred))] for pred in predictions]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bank_size', type=int, default=3000,
